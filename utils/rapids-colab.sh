...
if (( $RAPIDS_RESULT >=13 )) ;then
  ...
  echo "RAPIDS Version modified to 0.13 stable"
...
if [ ! -f Miniconda3-4.5.4-Linux-x86_64.sh ]; then
    ...
    # install RAPIDS packages
        conda install -y --prefix /usr/local \
                ...
                python=3.6 cudatoolkit=10.0 gcsfs pynvml xgboost \
                cudf=$RAPIDS_VERSION \
                cuml=$RAPIDS_VERSION \
    ...
    elif (( $RAPIDS_RESULT >= 12 )) ;then #Stable packages requiring PyArrow 0.15
